    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={stat.label} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
      ...
      {/* Alerts list */}
      {alerts.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-16 flex flex-col items-center text-center">
          <div className="w-20 h-20 rounded-full bg-green-50 flex items-center justify-center mb-5">
            <BellIcon className="w-10 h-10 text-[#6B9B5F]/50" />
          </div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Aucune alerte configurée</h3>
        ...
        <div className="space-y-4">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className={`bg-white dark:bg-gray-800 rounded-2xl border shadow-sm p-6 transition-all duration-200 hover:shadow-md ${
                alert.is_active ? 'border-green-100' : 'border-gray-100 dark:border-gray-700 opacity-75'
              }`}
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                    alert.is_active ? 'bg-green-50' : 'bg-gray-100 dark:bg-gray-700'
                  }`}>
                    <BellIcon className={`w-5 h-5 ${alert.is_active ? 'text-[#6B9B5F]' : 'text-gray-400 dark:text-gray-500'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-bold text-gray-900 dark:text-white truncate">{alert.name}</h3>
                      {alert.is_active ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-xs font-semibold text-gray-500 dark:text-gray-400">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  >
                    <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white dark:bg-gray-800 rounded-full shadow transition-transform duration-200 ${
                      alert.is_active ? 'translate-x-5' : 'translate-x-0'
                    }`} />
                  </button>
                  <button
                    onClick={() => handlePreview(alert)}
                    className="p-2 rounded-xl text-gray-400 dark:text-gray-500 hover:text-[#6B9B5F] hover:bg-green-50 transition-colors"
                    title="Prévisualiser"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-gray-400 dark:text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors"
                    title="Modifier"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-gray-400 dark:text-gray-500 hover:text-red-600 hover:bg-red-50 transition-colors"
                    title="Supprimer"
                  >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 rounded-t-3xl z-10">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              {/* Nom */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Nom de l&apos;alerte</label>
                <input
                  type="text"
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6B9B5F]/30 focus:border-[#6B9B5F] transition-colors"
                  value={formData.name}
              ...
              {/* Mots-clés */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Mots-clés <span className="text-gray-400 dark:text-gray-500 font-normal">(séparés par des virgules)</span></label>
                <input
                  type="text"
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6B9B5F]/30 focus:border-[#6B9B5F] transition-colors"
              ...
              {/* Localisation */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Localisation</label>
                <input
                  type="text"
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6B9B5F]/30 focus:border-[#6B9B5F] transition-colors"
              ...
              {/* Types de contrat */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Types de contrat</label>
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all duration-150 ${
                        formData.job_types.includes(jt.value)
                          ...
                          : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-[#6B9B5F] hover:text-[#6B9B5F]'
                      }`}
                    >
                      {jt.label}
                    </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Salaire min (FCFA)</label>
                  <input
                    type="number"
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6B9B5F]/30 focus:border-[#6B9B5F] transition-colors"
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Salaire max (FCFA)</label>
                  <input
                    type="number"
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#6B9B5F]/30 focus:border-[#6B9B5F] transition-colors"
              ...
              {/* Fréquence */}
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Fréquence d&apos;envoi</label>
            ...
            <div className="p-4 overflow-y-auto space-y-3">
              {previewJobs.length === 0 ? (
                <div className="text-center py-10 text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Essayez d&apos;élargir vos critères</p>
                </div>
              ) : previewJobs.map((job) => (
                <div key={job.id} className="p-4 rounded-2xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 hover:bg-green-50/30 transition-colors">
